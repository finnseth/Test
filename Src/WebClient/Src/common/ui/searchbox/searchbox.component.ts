...
import { SearchResult, SearchResultElement } from 'common/services/search/searchresult';
...
export class SearchboxComponent implements OnInit {
  ...
  onKeydown(event) {
    ...
    switch(event.which) {
      case 40: // Down
        var currentElementIndex = this.getElementIndex(this.selectedCategory, this.selectedElement);
        if (currentElementIndex < this.selectedCategory.elements.length - 1) {
          // Move down within current category
          this.selectedElement = this.selectedCategory.elements[currentElementIndex + 1];
        }
        else {
          // Current category exhausted, move to next category or jump to top
          var currentCategoryIndex = this.getCategoryIndex(this.selectedCategory);
          if (currentCategoryIndex < this.results.length - 1) {
            this.selectedCategory = this.results[currentCategoryIndex + 1];
          }
          else {
      ...
      case 38: // Up
        ...
        if (currentElementIndex > 0) {
          ...
          if (currentCategoryIndex > 0) {
            ...
            this.selectedCategory = this.results[this.results.length - 1];
          }
          this.selectedElement = this.selectedCategory.elements[this.selectedCategory.elements.length - 1];
        }
      ...
      case 13:
        ...
        this.searchValue = '';
        alert(this.selectedElement.name + ' => ' + this.selectedElement.route);
        event.preventDefault();
        break;
    ...
    getElementIndex(category: SearchResult, element: SearchResultElement) : number
    {
      let index: number = -1;
    ...
    selectDefaultCategoryAndElement() : void {
      if (this.selectedCategory == null) {
        if (this.results.length > 0) {
          this.selectedCategory = this.results[0];
          if (this.selectedCategory.elements && this.selectedCategory.elements.length > 0) {
            this.selectedElement = this.selectedCategory.elements[0];
          }
          else {
            this.selectedElement = null;
