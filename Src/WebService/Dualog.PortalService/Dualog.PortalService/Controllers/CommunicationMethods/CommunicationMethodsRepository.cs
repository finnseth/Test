...
{
    public class CommunicationMethodsRepository
    {
        readonly IDataContextFactory _dcFactory;

        public CommunicationMethodsRepository( IDataContextFactory dcFactory )
        {
            _dcFactory = dcFactory;
        }

        public async Task<IEnumerable<CommunicationMethodModel>> GetCommMethods( long companyId, long vesselId )
        {
        ...
        public async Task<CommunicationMethodDetailsModel> GetSingleComMethod( long companyId, long vesselId, long id )
        {
            using( var dc = _dcFactory.CreateContext() )
            {
                var q = from e in dc.GetSet<DsCarrierVesselConfig>()
                ...
                if( communicationMethodDetails == null )
                    ...
                    return communicationMethodDetails;
            }
        }
    }
}
